...
The web UI is available on port `8080` of the synth client deployment. When running with docker-compose, you can access it at:
...
- Source and destination racks
- Broker rack (via rack)
- Current latency in milliseconds with color coding (green for healthy, red for high latency)
...
- Multiple percentiles (median, 80th, 90th, 95th, 99th)
...
.Synth Client UI Ports for ack=all configuration
|===
|Instance |Metrics Port |Web UI Port
...
.Synth Client UI Ports for ack=1 configuration
|===
|Instance |Metrics Port |Web UI Port
