...
public class IndexRoute {
    ...
    void prerender() {
        try (var originalHtmlStream = getClass().getClassLoader().getResourceAsStream("templates/index.html")) {
            var rp = config.contextPath().endsWith("/") ? config.contextPath() : config.contextPath() + "/";
    ...
    void init(@Observes Router router) {
        Log.info("Initializing SPA routing for root path: " + config.contextPath());
        var rp = config.contextPath().endsWith("/") ? config.contextPath() : config.contextPath() + "/";
        if (!rp.equals("/")) {
            // Any path that is under the context path will be stripped of the context path and rerouted to the equivalent path without context
            router.getWithRegex(rp + ".*").handler(rc -> {
                var contextFreePath = rc.normalizedPath().substring(rp.length() - 1);
                rc.reroute(contextFreePath);
            });
        }
